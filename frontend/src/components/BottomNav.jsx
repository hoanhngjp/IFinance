...
export default function BottomNav() {
  ...
  const NavItem = ({ to, icon: Icon, label }) => {
    ...
    return (
      <Link to={to} className={`flex flex-col items-center justify-center w-14 transition-colors ${isActive ? 'text-indigo-600' : 'text-gray-400 hover:text-indigo-400'}`}>
  ...
  return (
    <div className="lg:hidden absolute bottom-0 w-full bg-white border-t border-gray-200 px-6 py-3 flex justify-between items-center z-50 rounded-t-2xl shadow-[0_-4px_15px_rgba(0,0,0,0.05)]">
      ...
      <Link to="/add" className="relative -top-6 bg-indigo-600 text-white p-4 rounded-full shadow-lg hover:bg-indigo-700 transition-transform transform hover:scale-105 active:scale-95">
        <PlusCircle size={32} />
      </Link>

      <NavItem to="/ai-chat" icon={MessageSquare} label="Trợ lý AI" />
