    ...
    <div className="min-h-screen bg-gray-50 pb-24 lg:p-8 animate-fade-in">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center bg-white p-6 rounded-3xl shadow-sm border border-gray-100">
          ...
          <button onClick={() => setIsAddModalOpen(true)} className="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2.5 rounded-2xl flex items-center gap-2 font-medium transition-all">
            ...
            return (
              <div key={debt.debt_id} className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all relative overflow-hidden group">
                ...
                <div className="mt-6 flex gap-3">
                  <button onClick={() => fetchHistory(debt)} className="p-3 bg-gray-50 hover:bg-gray-100 text-slate-600 rounded-xl transition-colors"><History size={20}/></button>
                  <button disabled={Number(debt.remaining_amount) === 0} onClick={() => { setSelectedDebt(debt); setIsRepayModalOpen(true); }} className={`flex-1 py-3 rounded-xl text-sm font-bold text-white transition-all shadow-sm disabled:opacity-30 ${activeTab === 'payable' ? 'bg-indigo-600 hover:bg-indigo-700' : 'bg-emerald-600 hover:bg-emerald-700'}`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden max-h-[80vh] flex flex-col">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                ...
                <button onClick={() => setIsHistoryModalOpen(false)} className="text-gray-400 bg-gray-50 p-2 rounded-full"><X size={20} /></button>
            </div>
                    ...
                    <div className="space-y-4">
                        {repaymentHistory.map(item => (
                            <div key={item.repayment_id} className="flex justify-between items-center p-3 border-b border-gray-50">
                                ...
                                <span className="text-xs bg-gray-100 px-2 py-1 rounded text-gray-500">{item.note}</span>
                            </div>
                        ))}
                    </div>
                )}
      ...
      {/* MODAL THÊM NỢ (CÓ DUE DATE) */}
      {isAddModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-slate-900/50 backdrop-blur-sm">
          <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                <h3 className="font-bold text-xl text-slate-800">Tạo khoản nợ mới</h3>
                <button onClick={() => setIsAddModalOpen(false)} className="text-gray-400 bg-gray-50 p-2 rounded-full"><X size={20} /></button>
            </div>
            <form onSubmit={handleCreateDebt} className="p-6 space-y-4">
              <input type="text" required value={newDebtData.creditor_name} onChange={(e) => setNewDebtData({...newDebtData, creditor_name: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none" placeholder="Tên đối tác..." />
              <CurrencyInput required value={newDebtData.total_amount} onChange={(e) => setNewDebtData({...newDebtData, total_amount: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl text-2xl font-bold" placeholder="Số tiền gốc" />
              <div>
                  <label className="text-xs font-bold text-gray-400 uppercase ml-1">Hạn thanh toán (Không bắt buộc)</label>
                  <input type="date" value={newDebtData.due_date} onChange={(e) => setNewDebtData({...newDebtData, due_date: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl mt-1 outline-none" />
              </div>
              <select required value={newDebtData.wallet_id} onChange={(e) => setNewDebtData({...newDebtData, wallet_id: e.target.value})} className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none">
                  <option value="">-- Chọn ví thực hiện --</option>
                  {wallets.map(w => <option key={w.wallet_id} value={w.wallet_id}>{w.name}</option>)}
              </select>
              <button disabled={isSubmitting} className="w-full bg-indigo-600 text-white py-4 rounded-xl font-bold shadow-lg shadow-indigo-100">Xác nhận</button>
            </form>
          </div>
        </div>
      )}
