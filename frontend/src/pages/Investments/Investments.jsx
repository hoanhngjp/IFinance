...
import { TrendingUp, Coins, Activity, Plus, TrendingDown, RefreshCw, CheckCircle2, X, Loader2, PieChart as PieChartIcon, HandCoins } from 'lucide-react';
...
export default function Investments() {
  ...
  // Forms
  const [buyForm, setBuyForm] = useState({ name: '', type: 'stock', principal_amount: '', fee: '', tax: '', wallet_id: '', start_date: new Date().toISOString().split('T')[0] });
  ...
  const handleBuy = async (e) => {
    ...
    try {
      await promise;
      setModalType(null);
      setBuyForm({ name: '', type: 'stock', principal_amount: '', fee: '', tax: '', wallet_id: '', start_date: new Date().toISOString().split('T')[0] });
      fetchData();
